...
\noindent  A truck can hold 25 containers (each of them is identical in shape and size). The
weight of the containers follows a normal distribution with mean 150 Kg and
standard deviation 15 Kg. \\

\noindent What is the probability that 25 containers will overload the
truck if the maximum load limit is 4000 Kg? You can use approximation.
...
\begin{itemize}
    \item Let $X_i$ denote the weight of i th container and let S= X1+X2+…+X25
    \item Given that $X ~ N(150,15^2)$
    \item Then, $E(S) = 25E(X_i)$ (since X s i
' are iid)
= 25 x 150 = 3750 Kgs.
Var(S) = 25Var(X1) = 25 x 152 = 5625
    \item Let Z S E S S S Z
s
3750 75
75
( ) 3750 ⇒ = +
...
=
σ
    \item If p is the probability of overloading the truck, then
\end{itemize}
...
&=& 0.0004\\
\end{eqnarray*}
On an average, the truck shall be overloaded 4 times in 10,000 occasions.